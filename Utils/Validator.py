...
class Validator:
    ...
    def validate_ext4(self, forward_function):
        ae_for_age_group = {x: [] for x in range(8)}
        ae_for_age = {x: [] for x in range(81)}
        ...
        return (sigma, ae_for_age, ae_for_age_group, mae, torch.max(torch.tensor(0), 5 - mmae) + torch.max(torch.tensor(0), 5 - sigma), torch.max(torch.tensor(0), 7 - mae) + torch.max(torch.tensor(0), 3 - sigma))